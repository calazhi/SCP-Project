          ...
          <template slot-scope="scope">
            ...
            <el-dropdown @command="handleCommand($event, scope.$index, scope.row)" trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="isStructuredNode && scope.row.fileUploadFlag && scope.row.versionStatus!='mm.versts.disable' && deployCondition.indexOf(scope.row.displayDeployStatus)>=0" class="model-dropdown-item" command="deploy"><i class="el-icon-tickets"></i>部署</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.versionStatus!='mm.versts.disable' && scope.row.displayDeployStatus == 'mm.depsts.deploy' && scope.row.displayPublishStatus != 'mm.pubsts.publish'" class="model-dropdown-item" command="publish"><i class="el-icon-upload2"></i>发布</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.versionStatus!='mm.versts.disable' && scope.row.displayPublishStatus == 'mm.pubsts.publish'" class="model-dropdown-item" command="excuteTask"><i class="el-icon-tickets"></i>调用模型</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.displayDeployStatus == 'mm.depsts.deploy'" class="model-dropdown-item" command="taskplan"><i class="el-icon-time"></i>查看计划</el-dropdown-item>
                <el-dropdown-item v-if="isStructuredNode && scope.row.displayDeployStatus == 'mm.depsts.deploy'" class="model-dropdown-item" command="task"><i class="el-icon-search"></i>查看任务</el-dropdown-item>
                ...
                <el-dropdown-item v-if="isStructuredNode && scope.row.fileUploadFlag && scope.row.versionStatus!='mm.versts.disable' && scope.row.displayDeployStatus=='mm.depsts.none'" class="model-dropdown-item" command="updatefile"><i class="el-icon-upload"></i>更新模型文件</el-dropdown-item>
                ...
                <el-dropdown-item v-if="isStructuredNode && scope.row.displayDeployStatus != 'mm.depsts.progress' && scope.row.displayPublishStatus != 'mm.pubsts.publish'" class="model-dropdown-item" command="delete" ><i class="el-icon-delete"></i>删除</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
      ...
      <el-form v-loading="loadingExcuteTask" :model="excuteVersionTask" ref="excuteVersionTask" label-width="40%" class="demo-ruleForm">
        ...
        <div v-if="currentTaskExeStep == 2" class="excute-min-height">
          ...
          <div style="height: 14px; line-height: 22px; padding: 0 0 0 15px;" class="red">
            ...
            <div v-if="isExeEmptyNode && currentDeployPublishAction==='excute'">请选择要调用模型的管理节点</div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      uploadFailed (err, file, fileList) {
        ...
        this.showUploadArea = true
        // clearInterval(this.uploadInterval)
        // this.afterUploadSendMessage(err)
      },
      closeAddModel () {
